...
def compute_coverage_loss(
    ...
    # Safety check: return zero tensor if loss is NaN or Inf
...
def combine_losses(
    ...
    # Add auxiliary losses with validation
    for key, aux_loss in aux_losses_dict.items():
        if aux_loss is not None and key in weights_dict:
            # Check if auxiliary loss is valid (not NaN or Inf)
            if torch.isnan(aux_loss).any() or torch.isinf(aux_loss).any():
                # Skip this auxiliary loss and log as 0.0
                loss_components[key] = 0.0
                loss_components[f"{key}_weighted"] = 0.0
                print(
                    f"[WARNING] {key} loss is NaN/Inf, skipping for this batch")
            else:
                # Valid loss: add to total and log
